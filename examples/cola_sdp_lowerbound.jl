#cola sdp w lower bound constraint 
...
#set the delta v limit per timestep. dv limit gets transformed to 
#acceleration limit by dividing by time.
#in scaled units
control_upper_limit = (0.01/(horizon[2]*scaling_units.time_scale))/scaling_units.acceleration_scale
control_lower_limit = ((0.008/(horizon[2]*scaling_units.time_scale))/scaling_units.acceleration_scale)/5.0
...
for k=1:N_period-1 
    ...
    push!(constraints, norm(utraj[:,k]) <= control_upper_limit)

    #lower bound constraint
    push!(constraints, tr(reshape(uutraj[:,k], nu, nu)) >= (control_lower_limit)^2)
...
#set the optimizer tolerance to 1e-7
optimizer = Mosek.Optimizer(MSK_DPAR_INTPNT_CO_TOL_PFEAS = 1e-7, MSK_DPAR_INTPNT_CO_TOL_DFEAS = 1e-7)
...
plot(all_e, xlabel="Knot Point", ylabel="Eigenvalue Ratio", title="Tightness Check", label="eigen value ratio",  yscale=:log10, ylim= (5e4, 10e8)) 
...
#plot the maneuver magnitude along with the bounds
...
plot!(horizon[1:N_period-1]*scaling_units.time_scale/T, control_upper_limit*scaling_units.acceleration_scale*1000*ones(N_period-1), label="upper limit", linestyle=:dash, color=:red)
plot!(horizon[1:N_period-1]*scaling_units.time_scale/T, control_lower_limit*scaling_units.acceleration_scale*1000*ones(N_period-1), label="lower limit", linestyle=:dash, color=:blue)
