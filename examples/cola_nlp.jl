#NLP formulation for Example 1 (case with no lower bound)
using Pkg 
...
function inequality_constraints(params, Z)
    ...
    for i=1:params.N-1
        inequality_c[i+1] = norm(Z[idx.u[i]]) -  ((0.04/(horizon[2]*scaling_units.time_scale))/scaling_units.acceleration_scale)
    end
...
function create_initial_guess(idx)
    ...
    for i=1:size(idx.u)[1]
        initial_guess[idx.u[i]] = 1e-6*ones(idx.nu)
    end
...
c_l = -Inf*ones(N_period)
c_u = Inf*ones(N_period)
